...
from utils.rtgs_utils import (
    downsample_frame_data,
    maybe_accumulate_mapping_grads,
    pose_delta_norm,
    prune_low_gradient_gaussians,
    select_tracking_scale,
)
...
def rgbd_slam(config: dict):
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        # Optimization Iterations (RTGS: protect init mapping budget)
        num_iters_mapping = config['mapping']['num_iters']
        if time_idx == 0 and rtgs.get("protect_init_mapping", True):
            num_iters_mapping = int(
                config['mapping'].get('num_iters_init', max(num_iters_mapping, 30))
            )
        ...
        # RTGS: adaptive tracking resolution (runtime downsample)
        if time_idx > 0 and rtgs.get("enable_adaptive_tracking_resolution", False):
            track_scale = select_tracking_scale(
                time_idx, params, params['means3D'].shape[0], config
            )
            tracking_curr_data = downsample_frame_data(tracking_curr_data, track_scale)
            if time_idx % 25 == 0:
                print(f"RTGS adaptive tracking scale={track_scale:.2f} "
                      f"at frame {time_idx} (gaussians={params['means3D'].shape[0]})")
        ...
        if time_idx > 0 and not config['tracking']['use_gt_poses']:
            ...
            # RTGS: soft tracking iter reduction
            if rtgs.get("reduce_tracking_iters", False):
                tr = float(rtgs.get("tracking_iter_ratio", 1.5))
                num_iters_tracking = max(
                    int(rtgs.get("tracking_min_iters", 40)),
                    int(round(num_iters_tracking / max(tr, 1.0))),
                )
            progress_bar = tqdm(range(num_iters_tracking), desc=f"Tracking Time Step: {time_idx}")
            early_stop = rtgs.get("enable_tracking_early_stop", False)
            min_track_iters = int(rtgs.get("tracking_min_iters", 40))
            pose_eps = float(rtgs.get("tracking_pose_eps", 3e-4))
            pose_patience = int(rtgs.get("tracking_converge_patience", 2))
            pose_hits = 0
            prev_rot = params['cam_unnorm_rots'][..., time_idx].detach().clone()
            prev_tran = params['cam_trans'][..., time_idx].detach().clone()
            while True:
                ...
                iter += 1
                if early_stop and iter >= min_track_iters:
                    dpose, prev_rot, prev_tran = pose_delta_norm(
                        params, time_idx, prev_rot, prev_tran
                    )
                    if dpose < pose_eps:
                        pose_hits += 1
                        if pose_hits >= pose_patience:
                            break
                    else:
                        pose_hits = 0
        ...
        # RTGS: optionally map only on keyframe cadence (map_every still applies)
        is_keyframe_frame = (
            (time_idx == 0)
            or ((time_idx + 1) % config["keyframe_every"] == 0)
            or (time_idx == num_frames - 2)
        )
        do_mapping = time_idx == 0 or (time_idx + 1) % config["map_every"] == 0
        if rtgs.get("map_only_on_keyframes", False) and time_idx > 0:
            do_mapping = do_mapping and is_keyframe_frame
        ...
        if do_mapping:
            ...
            for iter in range(num_iters_mapping):
                ...
                loss.backward()
                with torch.no_grad():
                    # RTGS: accumulate view-space grads for importance pruning
                    variables = maybe_accumulate_mapping_grads(variables, rtgs)
                    # Prune Gaussians
            ...
            if num_iters_mapping > 0:
                progress_bar.close()
            # RTGS: late low-gradient pruning after mapping
            with torch.no_grad():
                params, variables, removed = prune_low_gradient_gaussians(
                    params, variables, optimizer, time_idx, num_frames, rtgs
                )
                if removed:
                    print(
                        f"RTGS gradient prune at frame {time_idx}: "
                        f"removed {removed}, remaining {params['means3D'].shape[0]}"
                    )
            # Update the runtime numbers
    ...
    # Approx overall FPS: wall-clock over all frames (track+map per frame)
    total_slam_time = tracking_frame_time_sum + mapping_frame_time_sum
    overall_fps = num_frames / max(total_slam_time, 1e-6)
    print(f"\nAverage Tracking/Iteration Time: {tracking_iter_time_avg*1000} ms")
    print(f"Average Tracking/Frame Time: {tracking_frame_time_avg} s")
    print(f"Average Mapping/Iteration Time: {mapping_iter_time_avg*1000} ms")
    print(f"Average Mapping/Frame Time: {mapping_frame_time_avg} s")
    print(f"Overall SLAM FPS (track+map): {overall_fps:.4f}")
    print(f"Overall SLAM Wall Time: {total_slam_time:.2f} s")
    if config['use_wandb']:
        wandb_run.log({"Final Stats/Average Tracking Iteration Time (ms)": tracking_iter_time_avg*1000,
                       ...
                       "Final Stats/Overall FPS": overall_fps,
                       "Final Stats/step": 1})
