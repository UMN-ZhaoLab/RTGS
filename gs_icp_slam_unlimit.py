...
class GS_ICP_SLAM(SLAMParameters):
    def __init__(self, args):
        ...
        from utils.rtgs_utils import default_rtgs_cfg
        self.rtgs = default_rtgs_cfg(enabled=bool(getattr(args, "rtgs", False)))
        if self.rtgs.get("enable"):
            scale = float(self.rtgs.get("keyframe_freq_scale", 1.0))
            self.keyframe_freq = max(1, int(round(self.keyframe_freq * scale)))
            print(f"[RTGS] enabled: keyframe_freq={self.keyframe_freq}, "
                  f"adaptive_downsample={self.rtgs.get('enable_adaptive_downsample')}, "
                  f"map_throttle={self.rtgs.get('enable_mapping_throttle')}, "
                  f"grad_prune={self.rtgs.get('enable_gradient_pruning')}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rtgs", action="store_true", default=False,
                        help="Enable RTGS speedups (adaptive downsample, mapping throttle, pruning)")
