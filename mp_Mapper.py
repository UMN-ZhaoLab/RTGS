...
from utils.rtgs_utils import should_run_random_mapping, prune_low_gradient_gaussians
...
class Mapper(SLAMParameters):
    def __init__(self, slam):   
        ...
        self.rtgs = getattr(slam, "rtgs", {"enable": False})
        ...
        while True:
            ...
            if len(self.mapping_cams)>0:
                ...
                if len(self.new_keyframes) > 0:
                    ...
                    if not should_run_random_mapping(self.rtgs, is_new_keyframe=False):
                        time.sleep(1e-4)
                        continue
                ...
                loss.backward()
                with torch.no_grad():
                    if self.rtgs.get("enable") and self.rtgs.get("enable_gradient_pruning"):
                    ...
                    # RTGS late gradient pruning
                    if self.rtgs.get("enable"):
                        total_frames = max(len(self.trajmanager.gt_poses), 1)
                        frame_progress = float(self.iter_shared[0].item()) / float(total_frames)
                        pruned = prune_low_gradient_gaussians(
                            self.gaussians, frame_progress, self.train_iter, self.rtgs
                        )
                        if pruned > 0 and self.verbose:
                            print(f"[RTGS] pruned {pruned} gaussians @ iter {self.train_iter}")
                ...
                # Extra BA on new keyframes under RTGS (outside no_grad)
                if (
                    new_keyframe
                    and self.rtgs.get("enable")
                    and self.rtgs.get("enable_mapping_throttle")
                ):
                    extra = int(self.rtgs.get("map_new_kf_extra_iters", 2))
                    for _ in range(max(0, extra - 1)):
                        render_pkg2 = render_3(
                            viewpoint_cam, self.gaussians, self.pipe, self.background,
                            training_stage=self.training_stage,
                        )
                        image2 = render_pkg2["render"]
                        depth2 = render_pkg2["render_depth"]
                        vs2, vis2 = (
                            render_pkg2["viewspace_points"],
                            render_pkg2["visibility_filter"],
                        )
                        gt_image2 = viewpoint_cam.original_image.cuda() * mask
                        _, Ll1b = l1_loss(image2, gt_image2)
                        _, L_ssimb = ssim(image2, gt_image2)
                        _, Ll1_db = l1_loss(depth2 / d_max, gt_depth_image / d_max)
                        loss2 = (
                            (1.0 - self.lambda_dssim) * Ll1b
                            + self.lambda_dssim * (1.0 - L_ssimb)
                            + 0.1 * Ll1_db
                        )
                        loss2.backward()
                        with torch.no_grad():
                            if self.rtgs.get("enable_gradient_pruning") and vs2.grad is not None:
                                self.gaussians.add_densification_stats(vs2, vis2)
                            self.gaussians.optimizer.step()
                            self.gaussians.optimizer.zero_grad(set_to_none=True)
                        self.train_iter += 1
                    new_keyframe = False
