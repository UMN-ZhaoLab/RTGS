...
# import wandb  # Disabled to avoid dependency issues
    ...
    def __init__(self, config, save_dir=None):
        ...
        if self.config["Training"].get("single_thread", False):
            ...
            try:
                ...
                Log(f"Multiprocessing failed, falling back to single-threaded mode: {e}")
                self.config["Training"]["single_thread"] = True
                frontend_queue = FakeQueue()
                backend_queue = FakeQueue()
                q_main2vis = FakeQueue()
                q_vis2main = FakeQueue()
        ...
        # Start processes only if multiprocessing is available
...
if __name__ == "__main__":
    ...
    # Try to set multiprocessing start method, but fall back gracefully
    try:
    ...
    if args.eval:
        Log("Running MonoRTGS in Evaluation Mode")
