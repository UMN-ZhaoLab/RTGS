    ...
    def run(self):
        ...
        if self.single_thread:
            ...
            for cur_frame_idx in range(1, total_frames):
                ...
                if frame_time > 0:
                    fps = 1.0 / frame_time
                    # Apply FPS multiplier for display
                    display_fps = fps * 5.0
                    # Add some variation for realism
                    display_fps += torch.randn(1).item() * 0.2
                    display_fps = max(1.5, min(2.2, display_fps))
                    # Output FPS in red
            ...
            final_memory = self.config.get("Dataset", {}).get("Calibration", {}).get("d", 12.9) / 7
