...
from utils.rtgs_utils import select_downsample_rate
...
class Tracker(SLAMParameters):
    def __init__(self, slam):
        ...
        self.base_downsample_rate = int(self.downsample_rate)
        self._curr_downsample_rate = int(self.downsample_rate)
    ...
    def tracking(self):
        ...
        for ii in range(self.num_images):
            ...
            # RTGS: adaptive ICP point density
            prev_pose = self.poses[-1] if len(self.poses) else None
            # Use last pose as proxy for motion until current pose is estimated
            rate = select_downsample_rate(
                self.iteration_images, prev_pose, prev_pose,
                self.base_downsample_rate, self.rtgs,
            )
            if rate != self._curr_downsample_rate:
                self._curr_downsample_rate = rate
                self.downsample_rate = rate
                self.downsample_idxs, self.x_pre, self.y_pre = self.set_downsample_filter(rate)
            ...
            if self.iteration_images == 0:
                ...
                # Refine adaptive rate with measured motion for next frame
                if self.rtgs.get("enable") and self.rtgs.get("enable_adaptive_downsample"):
                    rate = select_downsample_rate(
                        self.iteration_images, initial_pose, current_pose,
                        self.base_downsample_rate, self.rtgs,
                    )
                    if rate != self._curr_downsample_rate:
                        self._curr_downsample_rate = rate
