...
# RTGS-accelerated SplaTAM on TUM freiburg1_desk
# Goal: raise FPS while keeping ATE/PSNR close to baseline SplaTAM.
...
# Fewer mapping calls + slightly sparser keyframes (RTGS-style)
map_every = 1
keyframe_every = 6
mapping_window_size = 18
tracking_iters = 200
mapping_iters = 20          # milder than ÷2
mapping_iters_init = 30
scene_radius_depth_ratio = 2

# Point basedir at local TUM data (override if needed)
# Expected: {basedir}/rgbd_dataset_freiburg1_desk/
_TUM_BASE = os.environ.get("SPLATAM_TUM_BASEDIR", "/mnt/hdd/datasets")

group_name = "TUM_RTGS"
run_name = f"{scene_name}_rtgs_seed{seed}"
...
config = dict(
    ...
    # ---- RTGS knobs (ported from MonoGS RTGS full-pipeline stack) ----
    rtgs=dict(
        ...
        tracking_iter_ratio=1.35,         # 200 -> ~148
